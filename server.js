...
io.on('connection', socket => {
  ...
  socket.on('join-room', (roomId, userId, userName) => {
    ...
    async function addNewRoom() {
      let userdb = await db.collection("users").find({ useremail: userId });
      userdb =  await userdb.toArray()
      if (!userdb.length) {
        console.log("here")
          userdb = new User({
          useremail: userId,
    ...
    }addNewRoom().catch(console.error)
    ...
    async function sendRooms() {
      var resultss = await db.collection("users").find({ useremail: userId });
      resultss =  await resultss.toArray();
      io.to(socket.id).emit('showRoomIds', resultss)
    } sendRooms().catch(console.error)

    //load messages of the given room
    async function getMessages() {
      var results = await db.collection("messages").find({ room: roomId });
